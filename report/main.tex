...
Pour pouvoir tester et commencer de vraies parties, nous avons créé deux premiers joueurs aléatoires : \fbox{move\_random}, un joueur qui se déplace aléatoirement, et \fbox{random}, un joueur qui se déplace et pose des murs de manière aléatoire. On rappelle que chaque coup joué par un joueur est défini en prenant en argument le mouvement du joueur adverse (voir Algorithme~\ref{algo:play}). \\
...
Ainsi, nous avons pu voir par l'exemple précédent qu'il y a au maximum 5 mouvements autorisés disponibles. Les joueurs aléatoires \fbox{move\_random} et \fbox{random} récupèrent ces mouvements et choisissent aléatoirement l'un de ces coups.
...
Le joueur \fbox{Usain Bolt} cherche alors à trouver le mouvement qui réduira au minimum cette valeur. Cette implémentation permet d'ordonner les mouvements de façon hiérarchique (voir figure~\ref{fig:rush1}).

\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.65]{rush1.png}
    \caption{Ordre de préférence des mouvements par le joueur \fbox{Usain Bolt}}
    \label{fig:rush1}
\end{figure}
...
\begin{figure}[h!t]
    ...
    \caption{Joueur \fbox{Usain Bolt} bloqué indéfiniment}
    \label{fig:rush2}
\end{figure}
...
Finalement, nous appelons la fonction \texttt{double\_dijkstra\_strategy} qui calcule les deux chemins pour les joueurs et décide de renvoyer un mouvement ou un mur en fonction des cas. Elle appelle toutes les fonctions cités plus haut et résume la stratégie de notre joueur \fbox{Rick}. \\
...
Le pseudo-algorithme de ce joueur est trouvable dans le fichier \texttt{white\_mage}. et nous nous y référerons par la suite par le nom \fbox{Mage blanc}.
